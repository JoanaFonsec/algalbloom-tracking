...
def main(args):
    ...
    with h5.File(args.path1, 'r') as f:
        position1 = f["traj"][()]
        measurements1 = f["measurement_vals"][()]
        gradient1 = f["grad_vals"][()]
        
    # Read h5 file of data 2
    with h5.File(args.path2, 'r') as f:
        position2 = f["traj"][()]
        measurements2 = f["measurement_vals"][()]
        gradient2 = f["grad_vals"][()]
        lon = f["lon"][()]
        lat = f["lat"][()]
        chl = f["chl"][()]
        #time = f["time"][()]
    ...
    # Call plotter class
    plotter = plot_comparison.Plotter(position1, gradient1, measurements1, position2, gradient2, measurements2, lon, lat, chl, chl_ref, int(meas_per), time_step)
